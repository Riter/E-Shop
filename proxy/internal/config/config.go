...
func LoadConfig() *Config {
    ...
    cfg := &Config{
        ...
        ServiceRoutes: map[string]string{
            "/search": getEnv("SEARCH_SERVICE_URL", "http:
            "/items":  getEnv("MANAGE_ITEM_CRUD_URL", "http:
            "/products":   getEnv("FACADE_URL", "http:
        },

        SearchServiceUrl:   getEnv("SEARCH_SERVICE_URL", "http:
		ManageItemCrudUrl:   getEnv("MANAGE_ITEM_CRUD_URL", "http:
		FacadeUrl:   getEnv("FACADE_URL", "http:
