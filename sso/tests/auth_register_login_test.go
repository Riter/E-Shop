...
func TestRegisterLogin_Login_HappyPasth(t *testing.T) {
	ctx, st := suite.New(t)
...
func TestRegister_FailCases(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectedErr: "email is nessesary",
		},
		{
...
func TestRegisterLogin_IsAdmin(t *testing.T) {
	...
	user_id := int64(claims["uid"].(float64))

	respIsAdmin, err := st.AuthClient.IsAdmin(ctx, &ssov1.IsAdminRequest{
		UserId: user_id,
	})
	require.NoError(t, err)
	require.False(t, respIsAdmin.GetIsAdmin())
